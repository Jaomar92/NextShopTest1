...
import { removeItemsFromCart, toggleCart } from "@/redux/features/cartSlice";
...
const Cart: FC = () => {
  ...
  const handleRemoveItem = (id: string) => {
    dispatch(removeItemsFromCart({ _id: id }));
  };
      ...
      >
        ...
        <div className={classNames.itemContainer}>
          {cartItems && cartItems.length > 0 ? (
            cartItems.map((item) => (
              <div className={cartItemClassNames.container} key={item._id}>
                <Image
                  width={100}
                  height={100}
                  alt={item.name}
